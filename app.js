...
app.get('/movies/', async (request, response) => {
  const getAllMovieQuery = `
       ...
       FROM
       movie;`
  const moviesArr = await db.all(getAllMovieQuery)
  response.send(
    moviesArr.map(moviename => convertMovieNametoPascalCase(moviename)),
  )
})
...
app.get('/directors/', async (request, response) => {
  const getDirectoryQuery = `
      ...
      *
      FROM
      director;`
  const moviesArray = await db.all(getDirectoryQuery)
  response.send(
    moviesArray.map(director => convertDirectorDetailsPascalCase(director)),
  )
})

const convertMovieNametPascalCase = dbObject => {
  return {
    movieName: dbObject.movie_name,
  }
}
...
app.get('/directors/:directorId/movies/', async (request, response) => {
  ...
  response.send(
    movies.map(movienames => convertMovieNametPascalCase(movienames)),
  )
})
